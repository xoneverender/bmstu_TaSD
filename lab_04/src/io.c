...
void main_menu(void)
{
    ...
    puts("3. Сравнение производительности стека на массиве и стека на списке.");
    puts("0. Выход.\n");
...
void stack_operations_menu(void)
{
    ...
    puts("3. Вычисление значения выражения из стека.");
    puts("4. Вычисление значения выражения из введенной строки.");
    puts("5. Просмотр стека.");
    puts("6. Просмотр адресов удаленных элементов.");
...
err_code_e process_actions(const action_e action, menu_e *menu, removed_t *removed, action_funcs_t *funcs, mode_e mode)
{
    ...
    switch (action)
    {
        case ADD_ELEM:
            ...
            puts(BOLD "Стек до добавления элемента:" RESET);
            ...
            {
                puts(BOLD "Стек после добавления элемента:" RESET);
                funcs->print(funcs->stack);
                puts(GREEN"\nДобавление элемента выполнено успешно."RESET);
            }
            break;
        case REMOVE_ELEM:
            puts("\nУДАЛЕНИЕ ЭЛЕМЕНТА ИЗ СТЕКА\n");

            puts(BOLD "Стек до удаления элемента:" RESET);
            funcs->print(funcs->stack);
            ...
            {
                puts(BOLD "Стек после удаления элемента:" RESET);
                funcs->print(funcs->stack);

                puts(GREEN"\nЭлемент успешно удален."RESET);
            }
            break;
        case CALCULATE_STACK:
            puts("\nВЫЧИСЛЕНИЕ ЗНАЧЕНИЯ ВЫРАЖЕНИЯ\n");

            if ((rc = operate_calculating_rpn(funcs->stack, funcs->pop, funcs->push, funcs->free, mode, true)))
                process_error(rc);
            else
                puts(GREEN"\nЗначение выражения вычислено успешно."RESET);
            break;
        case CALCULATE_STR:
            puts("\nВЫЧИСЛЕНИЕ ЗНАЧЕНИЯ ВЫРАЖЕНИЯ В ВЕДЕННОЙ СТРОКЕ\n");

            if ((rc = operate_calculating_rpn_from_string(funcs->pop, funcs->push, funcs->free, mode)))
                process_error(rc);
            else
                puts(GREEN"\nЗначение выражения вычислено успешно."RESET);
            break;
